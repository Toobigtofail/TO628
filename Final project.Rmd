...
train_indices <- 1:(floor(nrow(sd_norm)*0.8))
test_indices <- (length(train_indices)+1):nrow(sd_norm)
train_data <- sd_norm[train_indices,]
test_data <- sd_norm[test_indices,]
...
if (run_all_log_models) {
  log_model <- glm(match ~ ., data = train_data, family = "binomial")
  ...
  speed_formula <- colnames(sd_norm) %>%
    ...
    paste("match ~ ", .) %>%
    as.formula()
  log_model_step <- glm(match~ 1, data = train_data, family = binomial)
  log_model_step <- step(log_model_step, scope = (speed_formula), direction = "forward")
  # Boosted Logistic Model
  log_model_boosted <- caret::train(match ~ ., data = train_data,
                                    method = "LogitBoost",
...
```{r}
a1 = accuracy(log_prediction, test_data$match, "Log Prediction Plain", TRUE)
a2 = accuracy(log_step_prediction, test_data$match, "Log Step", TRUE)
a3 = accuracy(log_boosted_prediction, test_data$match, "Log Boosted", TRUE)
a4 = accuracy(tree_prediction, test_data$match, "CTree Regression", TRUE)
a5 = accuracy(tree_random_forest_prediction, test_data$match, "Random Forest Classification", TRUE)
a6 = accuracy(tree_ada_prediction, test_data$match, "Ada Boost Classification", TRUE)

acc_predictions = c(a1,a2,a3,a4,a5,a6)
names = c("Log Prediction Plain","Log Step","Log Boosted","CTree Regression","Random Forest Classification","Ada Boost Classification")
acc_mat <- data.frame(ModelName = names, accuracy = acc_predictions) %>% print
```

```{r}
# Graphic for the accuracy
